...
$(document).ready(function() {
    const $htmlElement = $('html'); // العنصر <html>
    const $radioButtons = $('input[name="font-size"]');

    // تحميل الإعدادات المحفوظة تلقائيًا
    const savedFontSize = localStorage.getItem('fontSize');
    if (savedFontSize) {
        $htmlElement.addClass(savedFontSize); // تطبيق حجم الخط المحفوظ
        // تعيين الـ radio button المختار
        $(`input[value="${savedFontSize}"]`).prop('checked', true);
    }

    // تغيير حجم الخط عند اختيار المستخدم
    $radioButtons.on('change', function() {
        const selectedSize = $(this).val();

        // إزالة جميع classes السابقة
        $htmlElement.removeClass('x-small small medium large x-large xx-large');

        // تطبيق class الجديد
        $htmlElement.addClass(selectedSize);
