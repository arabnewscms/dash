@if ($multiSelectRecord)
    ...
    <input class="form-check-input border selectID{{ $resourceName }}" type="checkbox" value="`+data.id+`">
...
@foreach ($fields as $field)
    @if ($field['show_rules']['showInIndex'])
        ...
        @elseif($field['type'] == 'image')
            ...
            return `
            <img class="img-fluid rounded-circle img-thumbnail" src="`+imageLink+`" alt="image"
                style="cursor: pointer;width:48px;height:48px" data-bs-toggle="modal"
        ...
        @elseif($field['type'] == 'video')
            ...
            return `
            <a href="#void" data-bs-toggle="modal" data-bs-target="#video_box_{{ $resourceName }}`+data.id+`"><i
        ...
        @elseif($field['type'] == 'audio')
            ...
            return `
            <a href="#void" data-bs-toggle="modal" data-bs-target="#audio_box_{{ $resourceName }}`+data.id+`"><i
                    class="fa-solid fa-file-audio"></i></a>
            `;
            }else{
            return '-';
            }
        ...
        @elseif($field['type'] == 'checkbox')
            ...
            @if (isset($field['trueVal']) && isset($field['falseVal']))
                ...
                return `<i class="fa-solid fa-xmark  fa-2x text-danger"></i>`;
                }else{
        ...
        @else
            ...
            return ' <i class="fa fa-recycle fa-1x" style="color:#c33"></i> ' +
            data.{{ !empty($field['attribute']) ? $field['attribute'] : 'id' }};
            }else{
...
var buttons = `
@if ($pagesRules['edit'])
    <a class="edit_relation_row" res_icon="{{ $resource['navigation']['icon'] }}"
        res_label="{{ $resource['navigation']['customName'] }}" res_name="{{ $resource['resourceName'] }}"
        record_id="`+data.id+`" class='m-1' href='javascript:void'><i class='fa-regular fa-pen-to-square'></i></a>
@endif
@if ($pagesRules['show'])
    <a class="show_relation_row" res_icon="{{ $resource['navigation']['icon'] }}"
        res_label="{{ $resource['navigation']['customName'] }}" res_name="{{ $resource['resourceName'] }}"
        record_id="`+data.id+`" class='m-1' href='javascript:void'><i class='fa-solid fa-eye'></i></a>
@endif
@if ($pagesRules['destroy'])
    <a href="#" action='{{ url($DASHBOARD_PATH . '/resource/' . $resource['resourceName']) }}/`+data.id+`'
        rowid="`+data.id+`" class="deleteRow{{ $resourceName }}" class='m-1'><i
...
@if ($pagesRules['restore'])
    if(data?.deleted_at != null){
    buttons +=`<a href="#void"
        action='{{ url($DASHBOARD_PATH . '/resource/' . $resource['resourceName']) }}/restore/`+data.id+`?lang={{ request('lang', app()->getLocale()) }}'
        rowid="`+data.id+`" class="restoreRow{{ $resourceName }}"><i class="fa fa-recycle fa-1x"
            style="color:#c33"></i><a>`;
            }
@endif
return buttons;
