    ...
    <div class="col-lg-{{ $col }} col-md-{{ $col }} col-sm-12 col-xs-12 box_{{ $belongsToAttr }}">
        ...
        <div class="form-group my-3 ">
            ...
            <select id="{{ $belongsToAttr . request('ajax_loading', '') }}" {{-- search Select2 Query Start --}}
                ...
                {{-- search Select2 Query End --}} name="{{ strtolower($belongsToAttr) }}"
			...
			{{ strtolower($belongsToAttr) }} {{ $errors->has($belongsToAttr) ? 'is-invalid' : '' }}"
                ...
                @if (!$checkIfNeedChildData)
                    @php
                        ...
                        $query = $query
                            ->where(function ($q) use ($model, $belongsToAttr, $belongsToModal) {
                    ...
                    @foreach ($query as $rmodel)
                        <option value="{{ $rmodel->id }}"
                            @if (!empty($model->{$belongsToAttr}) && !empty($model->{$belongsToAttr}?->id)) {{ $model->{$belongsToAttr}->id == $rmodel->id ? 'selected' : '' }}
			{{-- 	@elseif(!empty($model->{$belongsToModal}) && !empty($model->{$belongsToModal}->id))
				{{ $model->{$belongsToModal}->id == $rmodel->id?'selected':'' }} --}} @endif>
        ...
        window["get_{{ $belongsToAttr }}_data"] = function(parent = null, column = null) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    var selectedValue =
                        '{{ !empty($model->{$belongsToAttr}->id) ? $model->{$belongsToAttr}->id : '' }}';
