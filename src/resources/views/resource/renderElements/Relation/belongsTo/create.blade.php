@if ($field['show_rules']['showInCreate'])
    @php
        ...
        $checkIfNeedChildData =
            isset($field['fromParent']) &&
            isset($field['fromParent']['parent']) &&
            isset($field['fromParent']['column']);
    ...
    <div class="col-lg-{{ $col }} col-md-{{ $col }} col-sm-12 col-xs-12 box_{{ $belongsToAttr }}">
        ...
        <div class="form-group my-3">
            ...
            <select id="{{ $belongsToAttr }}{{ request('ajax_loading', '') }}" {{-- search Select2 Query Start --}}
                ...
                {{-- search Select2 Query End --}} name="{{ strtolower($belongsToAttr) }}" {{--  {{!empty(request('relationField.'.$field['attribute']))?'disabled':''}}  --}}
                ...
                @if (!empty(request('relationField.' . $field['attribute'])))
                    @foreach ($resource::$model::where('id', request('relationField.' . $field['attribute']))->get() as $model)
                        <option value="{{ $model->id }}" selected>{{ $model->{$resource::$title} }}</option>
                    @endforeach
            ...
            @if (empty(request('relationField.' . $field['attribute'])) &&
                ...
                @if (method_exists($resource::$model, 'trashed'))
                    <div class="form-check form-switch pt-1 d-block">
                        <input class="form-check-input" style="width: 26px;height: 14px;margin-left: -2.5em;"

                            type="checkbox" role="switch" name="withTrashed{{ $belongsToAttr }}" value="yes"
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        window["get_{{ $belongsToAttr }}_data"] = function(parent_id = null, column = null) {
            ...
            var withTrashed = $('#withTrashed{{ $belongsToAttr . request('ajax_loading', '') }}').is(
                ':checked') ? true : false;
            $.ajax({
                ...
                beforeSend: function() {
                    $('#{{ $belongsToAttr . request('ajax_loading', '') }}').prop('disabled',
                        true).html('');
                },
                success: function(data) {
                    ...
                    @if (!empty($field['child']) && isset($field['child']))
                        @foreach ($field['child'] as $child)
                            ...
                            $('#{{ $belongsToAttr . request('ajax_loading', '') }}').on(
                                'select2:select',
                                function(e) {
                                    // console.log(formId);
                                    var nextColumn = $(
                                    ...
                                    if (nextColumn != '' && nextParentValue != '') {
                                        get_{{ $child }}_data(nextParentValue,
                                            nextColumn);
                                    }
                                });
                        @endforeach
        ...
        @if ($checkIfNeedChildData)
            ...
            $('#{{ $field['fromParent']['parent'] . request('ajax_loading', '') }}').on('select2:select',
                function(e) {
                    var parentSelect =
                        '#{{ $field['fromParent']['parent'] . request('ajax_loading', '') }}';
            ...
            @if (!empty(request('relationField')) && !empty(request('relationField.' . $field['fromParent']['parent'])))
                ...
                get_{{ $belongsToAttr }}_data(
                    '{{ request('relationField.' . $field['fromParent']['parent']) }}',
                    '{{ $field['fromParent']['column'] }}');
            @endif
        @else
